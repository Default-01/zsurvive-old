...
sql.query(`CREATE DATABASE IF NOT EXISTS zsurvive;`, async (err, rows) => {
	...
	sql.query(
		`CREATE TABLE IF NOT EXISTS zsurvive.users (id varchar(50) DEFAULT NULL,data mediumtext, createdAt tinytext, premium tinyint DEFAULT NULL, premiumUntil tinytext, lastPremiumCoins tinytext, userName text, energyNotifAt tinytext, voteNotifAt tinytext, votes text, totalVotes int DEFAULT '0', bossesKilled int DEFAULT '0', coinsSpent int DEFAULT '0', resourcesFarmed int DEFAULT '0');`,
		async (err) => {
			if (err) throw err;
		}
	);
