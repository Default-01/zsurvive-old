	...
	async execute(message, args, lang) {
		...
		for (let i = 0; i < tiers.length; i++) {
			...
			if (member.roles.cache.has(element.role)) {
				if (user.premium !== element.tier) {
					// If user has a different premium tier compared to their role --> Add new premium.
		...
		if (tier === null && user.premium !== null) {
			// Removing premium if user has no role
		...
		if (user.premium !== null && tier) {
			...
			let embed = new MessageEmbed()
				.setTitle(`ZSurvive | Patreon`)
				.setDescription(
					`<@${message.author.id}> Thanks for contributing to ZSurvive's development by being a Patron!
			...
			**Collect them again on**: ${new Date(parseInt(user.lastPremiumCoins) + 1000 * 60 * 60 * 24 * 30).toUTCString()}`
			...
			let embed = new MessageEmbed()
				.setTitle(`ZSurvive | Patreon`)
				.setDescription(
					`<@${message.author.id}> ${lang.becomePremium}
...
function addPremium(message, tier) {
	return new Promise(async (resolve, reject) => {
		sql.query(
			`UPDATE ${envconfig.MySQL.database}.users SET premium = ?, premiumUntil = ? WHERE id = ? LIMIT 1;`,
			[tier.tier, Date.now() + 1000 * 60 * 60 * 24 * 30, message.author.id],
			async (err, result) => {
				if (err) throw err;
				resolve();
			}
