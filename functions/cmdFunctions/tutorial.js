...
async function insertUser(message) {
	return new Promise(async (resolve, reject) => {
		...
		sql.query(`SELECT * FROM ${envconfig.MySQL.database}.users WHERE id = ? LIMIT 1;`, message.author.id, async (err, rows) => {
			if (err) throw err;

			if (!rows[0]) {
				// If there is no user insert it

				sql.query(`INSERT INTO ${envconfig.MySQL.database}.users (id, data, createdAt) VALUES (?, ?, ?);`, [message.author.id, JSON.stringify(defaultData), now], async (err, rows) => {
					if (err) throw err;
