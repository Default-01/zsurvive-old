...
function getUser(message) {
	return new Promise((resolve, reject) => {
		sql.query(`SELECT id, data, premium, lastPremiumCoins FROM ${envconfig.MySQL.database}.users WHERE id = ? LIMIT 1;`, message.author.id, async (err, rows) => {
			if (err) throw err;
...
function updateUserData(data) {
	return new Promise((resolve, reject) => {
		...
		sql.query(
			`UPDATE ${envconfig.MySQL.database}.users SET data = ?, bossesKilled = ?, coinsSpent = ?, resourcesFarmed = ?, userName = ?, energyNotifAt = ? WHERE id = ? LIMIT 1;`,
			[JSON.stringify(data), data.stats.bossesKilled, data.stats.coinsSpent, data.stats.resourcesFarmed, name, data.stats.energyNotifAt, data.id],
			async (err, rows) => {
				if (err) throw err;
				return resolve();
			}
...
function getMaxHp(data) {
	...
	if (up) {
		for (let i = 0; i <= up.tier; i++) {
			// For with all completed and current tiers in hp
			...
			if (tier) {
				if (i < up.tier) hpmax += tier.amount * levels;
				else hpmax += tier.amount * up.level; // If tier is completed (== not the current one), add all levels to hp max.
			}
		}
	}
...
function getDmg(data) {
	...
	if (up) {
		for (let i = 0; i <= up.tier; i++) {
			...
			if (tier) {
				if (i < up.tier) dmg += tier.amount * levels;
				else dmg += tier.amount * up.level; // If tier is completed (== not the current one), add all levels to hp max.
			}
		}
	}
...
function getMaxEnergy(data) {
	...
	if (up) {
		for (let i = 0; i <= up.tier; i++) {
			...
			if (tier) {
				if (i < up.tier) max += tier.amount * levels;
				else max += tier.amount * up.level; // If tier is completed (== not the current one), add all levels to hp max.
			}
		}
	}
...
function getEnergy(data) {
	...
	if (data.energy >= data.maxenergy || data.notifDisabled) data.stats.energyNotifAt = null;
	else data.stats.energyNotifAt = now + remainingMS; // Notifications
