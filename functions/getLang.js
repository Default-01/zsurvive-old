...
function getLang(message) {
	...
	return new Promise(async (resolve, reject) => {
		...
		if (message.guild.id === '503243929559367710' && message.channel) {
			// Multi language for ZSurvive guild
		...
		sql.query(`SELECT * FROM ${envconfig.MySQL.database}.guilds WHERE guild = ?;`, message.guild.id, async (err, rows) => {
			if (err) throw err;
...
function updateLang(guildid, lang) {
	return new Promise((resolve, reject) => {
		...
		sql.query(`UPDATE ${envconfig.MySQL.database}.guilds SET lang = ? WHERE guild = ?;`, [lang, guildid], async (err, result) => {
			if (err) throw err;
			if (result.affectedRows > 0) return resolve();

			sql.query(`INSERT INTO ${envconfig.MySQL.database}.guilds (guild, lang) VALUES (?, ?);`, [guildid, lang], async (err, rows) => {
				if (err) throw err;
				return resolve();
			});
		});
...
setInterval(() => {
	// Clean cache every 15m. All server that were not used in the last 30m
