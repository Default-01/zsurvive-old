...
async function sendNotifications() {
	...
	sql.query(`SELECT id, data, energyNotifAt, voteNotifAt FROM ${envconfig.MySQL.database}.users WHERE energyNotifAt < ? OR voteNotifAt < ?;`, [now, now], async (err, rows) => {
		if (err) throw err;
		...
		rows.forEach(async (element) => {
			...
			if (!data.notifDisabled) {
				...
				if (element.energyNotifAt !== null && element.energyNotifAt < now && data.energy >= data.maxenergy) {
					// Energy notification

					data.stats.energyNotifAt = null;

					sql.query(`UPDATE ${envconfig.MySQL.database}.users SET data = ?, energyNotifAt = ? WHERE id = ? LIMIT 1;`, [JSON.stringify(data), null, data.id], async (err, rows) => {
						if (err) throw err;
					});
				...
				if (element.voteNotifAt !== null && element.voteNotifAt < now) {
					// Vote notification

					sql.query(`UPDATE ${envconfig.MySQL.database}.users SET voteNotifAt = ? WHERE id = ? LIMIT 1;`, [null, data.id], async (err, rows) => {
						if (err) throw err;
					});
				...
				sql.query(`UPDATE ${envconfig.MySQL.database}.users SET energyNotifAt = ? WHERE id = ? LIMIT 1;`, [null, data.id], async (err, rows) => {
					if (err) throw err;
				});
