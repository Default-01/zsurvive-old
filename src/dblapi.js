...
dbl.webhook.on('vote', (vote) => {
	sql.query(`SELECT * FROM ${envconfig.MySQL.database}.users WHERE id = ? LIMIT 1;`, vote.user, async (err, rows) => {
		if (err) throw err;
		...
		let userVotes;
		if (!rows[0].votes) userVotes = [];
		else userVotes = JSON.parse(rows[0].votes);

		userVotes.push(now);
		data.coins += coinsReward;

		sql.query(
			`UPDATE ${envconfig.MySQL.database}.users SET data = ?, voteNotifAt = ?, votes = ?, totalVotes = ? WHERE id = ? LIMIT 1;`,
			[JSON.stringify(data), now + 1000 * 60 * 60 * 12, JSON.stringify(userVotes), rows[0].totalVotes + 1, vote.user],
			async (err, rows) => {
				if (err) throw err;
