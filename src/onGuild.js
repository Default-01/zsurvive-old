...
function addToDB(guild, joined) {
	// Track invite or kick in database for charts.js

	sql.query(`INSERT INTO ${envconfig.MySQL.database}.invites (guild, date, joined) VALUES (?, ?, ?);`, [guild, new Date().getTime(), joined], async (err, rows) => {
		if (err) throw err;
	});
}
...
if (client.shard.ids[0] === 0) {
	async function updateGuildCount() {
		// Track guild count in database for charts.js

		let count = await client.shard.fetchClientValues('guilds.cache.size');
		count = count.reduce((prev, guildCount) => prev + guildCount, 0);

		sql.query(`INSERT INTO ${envconfig.MySQL.database}.guild_count (date, amount) VALUES (?, ?);`, [new Date().getTime(), count], async (err, rows) => {
			if (err) throw err;
		});
	}
